...
export const store = {
    ...
    async loadPaletteAndClusters() {
        ...
        try {
            ...
            if (!paletteResponse.ok) throw new Error(`Failed to load palette for ${variant}`);
            this._state.palette = await paletteResponse.json();
            
            // Load clusters
            const clustersResponse = await fetch(config.dataPaths.getClustersPath());
            if (!clustersResponse.ok) throw new Error(`Failed to load clusters for ${variant}`);
            ...
            if (this._subscribers['palette']) {
                this._subscribers['palette'].forEach(callback => callback(this._state.palette));
            }
            if (this._subscribers['clusters']) {
                this._subscribers['clusters'].forEach(callback => callback(this._state.clusters));
            }
        } catch (error) {
    ...
    _initUIBindings() {
        try {
            ...
            this.bindInputWithLabel('nucleiOpacity', 'nuclei-opacity-input', 'nuclei-opacity-value');
