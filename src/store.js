...
export const store = {
  ...
  _state: {
    ...
    geneChangeDebounceMs: 150, // Debounce time in milliseconds
  ...
  /**
   * Load palette and clusters data for the current variant
   */
  async loadPaletteAndClusters() {
    ...
    try {
      ...
      if (!paletteResponse.ok)
        throw new Error(`Failed to load palette for ${variant}`);
      this._state.palette = await paletteResponse.json();

      // Load clusters
      const clustersResponse = await fetch(config.dataPaths.getClustersPath());
      if (!clustersResponse.ok)
        throw new Error(`Failed to load clusters for ${variant}`);
      this._state.clusters = await clustersResponse.json();

      // console.log(`Loaded palette and clusters for ${variant}`);

      // Notify subscribers if any
      if (this._subscribers["palette"]) {
        this._subscribers["palette"].forEach((callback) =>
          callback(this._state.palette)
        );
      }
      if (this._subscribers["clusters"]) {
        this._subscribers["clusters"].forEach((callback) =>
          callback(this._state.clusters)
        );
      }
    } catch (error) {
      console.error("Error loading palette and clusters:", error);
      throw error;
    }
  },
  ...
  _initUIBindings() {
    try {
      ...
      this.bindInputWithLabel(
        "boundaryOpacity",
        "boundary-opacity-input",
        "boundary-opacity-value"
      );
      this.bindInputWithLabel(
        "boundarySubsample",
        "boundary-subsample-input",
        "boundary-subsample-value"
      );
      this.bindInputWithLabel(
        "intensityMin",
        "intensity-min-input",
      ...
      this.bindInputWithLabel(
        "innerColoringOpacity",
        "inner-coloring-opacity-input",
        "inner-coloring-opacity-value"
      );

      // Cell nuclei opacity binding
      this.bindInputWithLabel(
        "nucleiOpacity",
        "nuclei-opacity-input",
        "nuclei-opacity-value"
      );
      ...
      this.bindCheckbox("showCellNuclei", "show-nuclei-checkbox");
      ...
      this.subscribe("pointsRendered", (points) =>
        this.updateMetric("points-rendered-value", points.toLocaleString())
      );
  ...
  bindInputWithLabel(key, inputId, labelId) {
    ...
    const defaultValues = {
      ...
      intensityMin: 0,
      intensityMax: 255,
      innerColoringOpacity: config.visualization.defaultInnerColoringOpacity,
    };
    ...
    input.addEventListener("input", () => {
      ...
      if (!isNaN(value)) {
        // Apply point size range constraints from config
        if (key === "pointSize") {
          const range = config.visualization.pointSizeRange;
          value = Math.max(range.min, Math.min(range.max, value));
          input.value = value; // Update input to constrained value
        }
    ...
    this.subscribe(key, (value) => {
      ...
      if (label) {
        label.textContent = key === "pointSize" ? value.toFixed(1) : value;
      }
    });
  },
